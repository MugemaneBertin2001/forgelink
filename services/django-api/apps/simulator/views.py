...
class SimulatedPLCViewSet(viewsets.ModelViewSet):
    ...
    def bulk_control(self, request):
        ...
        if action_name == 'start':
            plcs.update(is_simulating=True, is_online=True, last_heartbeat=timezone.now())
            SimulatedDevice.objects.filter(plc__in=plcs).update(status='running', quality='good')
        elif action_name == 'stop':
...
class SimulatedDeviceViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['status', 'quality', 'fault_type', 'simulation_mode', 'plc', 'plc__area']
    ...
    def inject_fault(self, request, pk=None):
        ...
        SimulationEvent.objects.create(
            ...
            message=f"Fault injected: {fault_type}" if fault_type != 'none' else "Device recovered from fault",
    ...
    def bulk_control(self, request):
        ...
        if action_name == 'start':
            ...
            devices.update(
                ...
                fault_type='none', sequence_number=0, error_count=0
            )
...
class SimulationSessionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        serializer.save(created_by=self.request.user.email if hasattr(self.request, 'user') else '')
    ...
    def stats(self, request, pk=None):
        ...
        return Response({
            ...
            'duration': str(session.stopped_at - session.started_at).split('.')[0] if session.stopped_at else None,
...
class SimulationEventViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['event_type', 'severity', 'acknowledged', 'device', 'plc', 'session']
...
class SimulatorDashboardViewSet(viewsets.ViewSet):
    ...
    def overview(self, request):
        ...
        total_messages = SimulatedDevice.objects.aggregate(
    ...
    def areas(self, request):
        ...
        for area_code, area_display in SimulatedPLC.Area.choices:
            ...
            summary = {
                ...
                'avg_value': devices.filter(
                    current_value__isnull=False
                ).aggregate(avg=Avg('current_value'))['avg']
            }
    ...
    def live_values(self, request):
        ...
        devices = SimulatedDevice.objects.filter(
            status='running'
        ).select_related('plc', 'profile').values(
            'id', 'device_id', 'name',
            'current_value', 'quality', 'last_published_at',
            'plc__area', 'plc__line', 'profile__unit', 'profile__sensor_type'
        )
    ...
    def start_all(self, request):
        ...
        device_count = SimulatedDevice.objects.update(
            status='running',
            quality='good'
        )

        return Response({
            'plcs_started': plc_count,
            'devices_started': device_count
        })
