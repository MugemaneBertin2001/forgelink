...
class DeviceProfileAdmin(ModelAdmin):
    ...
    list_display = [
        'name', 'sensor_type_display', 'value_range_display', 'unit',
        'noise_display', 'device_count', 'created_at'
    ]
    list_filter = [
    ...
    fieldsets = (
        ...
        ('Physics Simulation', {
            'fields': ('noise_factor', 'drift_rate', 'response_time_ms', 'dead_band'),
            'classes': ['tab'],
        }),
        ('Thresholds', {
            'fields': ('low_threshold', 'high_threshold', 'critical_low', 'critical_high'),
            'classes': ['tab'],
        }),
...
class SimulatedPLCAdmin(ModelAdmin):
    ...
    list_display = [
        'name', 'plc_type_display', 'area_display', 'line', 'cell',
        'status_display', 'device_count', 'last_heartbeat'
    ]
    list_filter = [
    ...
    readonly_fields = ['id', 'created_at', 'updated_at', 'last_heartbeat', 'topic_prefix', 'opc_path']
    ...
    fieldsets = (
        ...
        ('Communication', {
            'fields': ('scan_rate_ms', 'publish_rate_ms', 'ip_address', 'firmware_version'),
            'classes': ['tab'],
        }),
    )
    ...
    def start_simulation(self, request, queryset):
        count = queryset.update(is_simulating=True, is_online=True, last_heartbeat=timezone.now())
...
class SimulatedDeviceAdmin(ModelAdmin):
    ...
    list_display = [
        'device_id', 'name', 'plc', 'profile_display',
        'status_display', 'current_value_display', 'quality_display',
        'fault_display', 'messages_sent'
    ]
    list_filter = [
    ...
    readonly_fields = [
        ...
        'last_published_at', 'last_value_change_at', 'error_count',
        'last_error', 'created_at', 'updated_at', 'mqtt_topic', 'opc_node_id',
        'effective_min', 'effective_max', 'value_range'
    ]
    ...
    fieldsets = (
        ...
        ('Current State', {
            'fields': ('status', 'current_value', 'target_value', 'quality', 'sequence_number'),
            'classes': ['tab'],
        }),
        ('Topic/Node Mapping', {
        ...
        ('Value Configuration', {
            'fields': ('min_value_override', 'max_value_override', 'noise_override',
                      'effective_min', 'effective_max', 'value_range'),
            'classes': ['tab'],
        }),
        ('Simulation Behavior', {
            'fields': ('simulation_mode', 'sine_period_seconds', 'ramp_rate_per_second'),
            'classes': ['tab'],
        }),
        ('Fault Injection', {
        ...
        ('Statistics', {
            'fields': ('messages_sent', 'last_published_at', 'last_value_change_at',
    ...
    actions_detail = [
        ...
        'inject_stuck_fault', 'inject_drift_fault', 'inject_spike_fault', 'clear_device_fault'
    ]
    actions = [
        'start_devices', 'stop_devices',
        'inject_fault_stuck', 'inject_fault_drift', 'inject_fault_spike', 'clear_faults'
    ]
    ...
    def inject_fault_stuck(self, request, queryset):
        count = queryset.update(fault_type='stuck', fault_start=timezone.now(), quality='bad')
    ...
    def inject_fault_drift(self, request, queryset):
        count = queryset.update(fault_type='drift', fault_start=timezone.now(), quality='uncertain')
    ...
    def inject_fault_spike(self, request, queryset):
        count = queryset.update(fault_type='spike', fault_start=timezone.now(), quality='uncertain')
    ...
    def clear_faults(self, request, queryset):
        count = queryset.update(fault_type='none', fault_start=None, fault_end=timezone.now(), quality='good')
...
class SimulationSessionAdmin(ModelAdmin):
    ...
    list_display = [
        'name', 'status_display', 'device_count', 'plc_count',
        'messages_sent', 'events_generated', 'duration_display', 'created_at'
    ]
    list_filter = [
        ('status', ChoicesDropdownFilter),
        ('created_at', RangeDateFilter),
    ...
    search_fields = ['name', 'description']
    readonly_fields = [
        'id', 'started_at', 'stopped_at', 'messages_sent',
        'events_generated', 'faults_injected', 'error_count',
    ...
    fieldsets = (
        ...
        ('Statistics', {
            'fields': ('messages_sent', 'events_generated', 'faults_injected', 'error_count', 'last_error'),
            'classes': ['tab'],
        }),
    )
...
class SimulationEventAdmin(ModelAdmin):
    ...
    list_display = [
        'created_at', 'event_type_display', 'severity_display',
        'device', 'value_display', 'acknowledged_display'
    ]
    list_filter = [
    ...
    def acknowledge_event(self, request, object_id):
        ...
        event.acknowledged_by = request.user.username if hasattr(request, 'user') else 'admin'
    ...
    def acknowledge_events(self, request, queryset):
        count = queryset.filter(acknowledged=False).update(
            acknowledged=True,
            acknowledged_by=request.user.username if hasattr(request, 'user') else 'admin',
    ...
    def unacknowledge_events(self, request, queryset):
        count = queryset.update(acknowledged=False, acknowledged_by='', acknowledged_at=None)
