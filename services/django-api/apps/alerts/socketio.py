...
class AlertNamespace(socketio.AsyncNamespace):
    ...
    async def on_connect(self, sid: str, environ: dict, auth: Optional[dict] = None):
        ...
        if not token:
            ...
            await self.emit(
                "auth:error", {"message": "Authentication required"}, to=sid
            )
        ...
        if not payload:
            ...
            await self.emit(
                "auth:error", {"message": "Invalid or expired token"}, to=sid
            )
    ...
    async def on_subscribe_area(self, sid: str, data: dict):
        ...
        if user_area and user_area != area:
            await self.emit(
                "error", {"message": f"Access denied for area: {area}"}, to=sid
            )
            return
    ...
    async def on_acknowledge(self, sid: str, data: dict):
        ...
        if (
            "alerts.acknowledge" not in permissions
            and session.get("role") != "FACTORY_ADMIN"
        ):
            await self.emit(
                "error",
                {"message": "Permission denied: alerts.acknowledge required"},
                to=sid,
            )
            return
        ...
        if alert:
            ...
            await self.emit(
                "error", {"message": "Alert not found or not active"}, to=sid
            )
