...
from apps.telemetry.kafka_consumer import TelemetryKafkaConsumer, EventKafkaConsumer
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Starting {consumer_type} consumer...')
        )
    ...
    def _run_event_consumer(self, options, bootstrap_servers):
        ...
        self.stdout.write(f'Topics: {", ".join(topics)}')
        self.stdout.write(self.style.SUCCESS('Event consumer started, press Ctrl+C to stop'))
    ...
    def _run_both_consumers(self, options, bootstrap_servers):
        ...
        telemetry_thread = threading.Thread(
            target=telemetry_consumer.start,
            name='telemetry-consumer',
            daemon=True
        )
        event_thread = threading.Thread(
            target=event_consumer.start,
            name='event-consumer',
            daemon=True
        )
        ...
        self.stdout.write(self.style.SUCCESS('Both consumers started, press Ctrl+C to stop'))
